...
def rebuild_manual_toc_black(in_file, out_file):
    doc = docx.Document(in_file)
    
    # 1. Remove the existing TOC heading and TOC entries
    paras_to_remove = []
    for i, p in enumerate(doc.paragraphs):
        ...
        # Check for hyperlink with anchor starting with TOC_BM_
    ...
    def create_toc_entry_para(text, level, bookmark_name):
        ...
        # Add tab stop for page number (even if we don't have page numbers, it formats it like a TOC)
        ...
        # Sizes: Chapter 14pt (28hps), H2 12pt (24hps), H3 12pt (24hps)
        ...
        # We can add a PAGEREF field inside the hyperlink to dynamically fetch the page number!
        # This will make it a TRUE manual TOC with accurate page numbers that update automatically!
        ...
        # Default text (e.g. "0") until updated
        ...
        # Append all to hyperlink
        hl.append(hl_r)
        hl.append(tab_r)
        hl.append(r_fld_begin)
        hl.append(r_instr)
        hl.append(r_fld_sep)
        hl.append(r_def)
        hl.append(r_fld_end)
    ...
    print(f"Inserted manual TOC with {len(toc_entries)} clickable hyperlinks in BLACK text with PAGEREFs")
    ...
    try:
        ...
        if updateFields is None:
            ...
            print("Set updateFields to true in settings (for the PAGEREFs).")
...
rebuild_manual_toc_black(r'd:\RESEARCH PNGS\zukhruftheisis_FINAL_TOC.docx', r'd:\RESEARCH PNGS\zukhruftheisis_Black_TOC.docx')
