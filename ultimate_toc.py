...
def create_ultimate_toc(in_file, out_file):
    ...
    def create_toc_entry_para(text, level, bookmark_name):
        ...
        # --- A. HYPERLINK (Text only, blue and underlined for visibility) ---
        ...
        sz_hl.set(qn('w:val'), str(font_size_hpc))
        rPr_hl.append(sz_hl)
        szCs_hl = OxmlElement('w:szCs')
        szCs_hl.set(qn('w:val'), str(font_size_hpc))
        rPr_hl.append(szCs_hl)
        if is_bold:
            rPr_hl.append(OxmlElement('w:b'))
            
        # explicitly blue and underlined
        ...
        # --- B. PAGE NUMBER (Black text, outside hyperlink) ---
        def make_black_rPr():
        ...
        # Default text (just an empty space or "1" until they update)
        r_def = OxmlElement('w:r')
        r_def.append(make_black_rPr())
    ...
    # Ensure updateFields is set so page numbers populate!
    try:
...
create_ultimate_toc(r'd:\RESEARCH PNGS\zukhruftheisis_cleaned.docx', r'd:\RESEARCH PNGS\zukhruftheisis_ULTIMATE_TOC.docx')
